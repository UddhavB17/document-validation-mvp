...
export default function ApplicationReviewPage() {
  ...
  // Polling diet: the full review is fetched once; the lightweight status
  // hook is the only poller, and the review refetches when it turns terminal.
  const applicationStatus = useApplicationStatus(isValidApplicationId ? applicationId : null);
  ...
  useEffect(() => {
    const liveStatus = applicationStatus.data?.status;
    if (liveStatus && !isApplicationReviewPollingStatus(liveStatus) && applicationReview.data) {
      const reviewStatus = applicationReview.data.application?.status;
      if (reviewStatus && isApplicationReviewPollingStatus(reviewStatus)) {
        void applicationReview.refetch();
      }
    }
