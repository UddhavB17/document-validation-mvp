...
import { getFormError, escapeControlCharacters } from "./uploadUtils";
import { UploadFormProps } from "./types";

export function PartnerJsonForm({ onUploaded }: UploadFormProps) {
  ...
  return (
    <form className="space-y-5" onSubmit={submit}>
      ...
      <button type="submit" disabled={isSubmitting} className="px-5 py-2.5 text-sm font-semibold rounded-lg bg-[#2B4C7E] hover:bg-[#1E3559] text-white transition-all duration-150 shadow-3xs disabled:bg-slate-200 disabled:text-slate-500 border-none cursor-pointer">
