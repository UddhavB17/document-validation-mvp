    ...
    <div className="rounded-xl border border-[#EAF0F8] bg-[#EAF0F8]/30 p-4 shadow-3xs">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between font-semibold">
        ...
        <div className="flex shrink-0 gap-2 text-xs font-semibold">
          <button type="button" onClick={openPreview} disabled={!objectUrl} className="rounded-lg bg-[#2B4C7E] hover:bg-[#1E3559] px-4 py-2 font-bold text-white shadow-3xs disabled:bg-slate-200 disabled:text-slate-500 cursor-pointer border-none">
            Open PDF Preview
          </button>
          <button type="button" onClick={onRemove} className="rounded-lg border border-[#E1E5EB] bg-white px-4 py-2 font-bold text-[#5C6B7A] hover:bg-slate-50 hover:text-[#16202E] cursor-pointer">
            Remove
          </button>
        </div>
      </div>
