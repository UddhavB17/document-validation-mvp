...
export function ReviewerSummary({ data, onSelectPage }: { data: ApplicationReview; onSelectPage?: (pageNo: number) => void }) {
  const summary = data.reviewer_summary;
  if (!summary) {
    return null;
  }
  const status = String(summary.overall_status ?? "LIMITED_REVIEW");
  const pagesToReview = Array.isArray(summary.pages_to_review)
    ? summary.pages_to_review.filter((n): n is number => typeof n === "number" || !isNaN(Number(n))).map(Number)
    : [];
  return (
    <section className="bg-slate-50 border border-slate-200 rounded-xl p-5 space-y-3">
      <h2 className="text-base font-bold text-slate-800">Reviewer Action Summary</h2>
