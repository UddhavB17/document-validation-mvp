...
import { ApplicantComparison, ComparisonStatus, countAttentionFields, normalizeApplicantList } from "@/lib/types";
import { asText } from "@/lib/format";

export function getApplicantList(data: ApplicationReview): ApplicantComparison[] {
  return normalizeApplicantList(data.comparison_matrix?.applicants);
}

export function getAnomalyCount(data: ApplicationReview): number {
  return countAttentionFields(data.comparison_matrix);
}
        ...
        <table className="w-full border-collapse text-left text-[13px]">
          ...
          <tbody className="divide-y divide-[#E1E5EB] text-[#16202E]">
            {applicantList.map((applicant) => {
              ...
              const status: ComparisonStatus = hasMismatch ? "mismatch" : hasAttention ? "attention" : "match";
